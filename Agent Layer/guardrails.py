...
class PIIDetector:
    """
    Scans text for PII before graph insertion.
    This is a placeholder regex approach. In production, replace with a
    trained NER model or a service like AWS Comprehend / Presidio.
    """
    ...
    def scan(self, text: str) -> list[dict]:
        """Returns list of detected PII items with type and span."""
        findings: list[dict] = []
        ...
        if findings:
            logger.info(f"PII detected: {len(findings)} item(s) flagged")
        return findings
